# Из 16-ричного числа в массив из строк чисел вида 0xчисло, т.е восмибитное число
def bytestringtolist(m):
    arrya = [0]*64
    arrya = [m[i:i+2] for i in range(0, len(m), 2)]
    arrya = [f'0x{j}' for j in arrya]
    return arrya


# Переводим массив строк в массив чиел
def liststringtoint(arrya):
    m = [int(value, 16) for value in arrya]
    return m

# Из 16-ричнго в двоичное
def hextobin(hex_number):
    binnum = ""
    for number in hex_number:
        bin_num = bin(int(number, 16))[2:].zfill(4)
        binnum += bin_num
    return binnum

# Из двоичного в 16-ричное
def bintohex(bin_number):
    hexnum = ""
    for i in range(0, len(bin_number)-3 , 4):
        hexnum += str(hex(int(bin_number[i:i+4], 2)))[2:]
    return hexnum

# Массив бинарной матрицы А
A = [
        ...
        for k in range(8):
            for j in range(8):
                if((a[i*8+k] & (1 << (7-j))) != 0):
                    c ^= A[k * 8 + j]
        for k in range(8):
            arr[i * 8 + k] = (c & 0xFF << (7 - k)*8) >> (7 - k)*8
    return arr

# Последовательность преобразований L, P и S
def LPS(state):
    state = L(P(S(state)))
    return state

# Последовательность преобразований L, P, S и X
def LPSX(state, b):
    state = LPS(xorall(state,b))
    return state

# Генерация итерационных ключей
def KeyScedule(K, i:int):
    K = LPSX(K, C[i])
    return K
...
# Преобразование чисел в шестнадцатиричную строку
def stroutput(state):
    hex_array = [hex(x) for x in state]
    result = ''.join([value[2:] if len(value[2:]) == 2 else "0" + value[2:] for value in hex_array])
    return result

# Преобразование в массив шестнадцатиричных чисел
def hexoutput(state):
    hex_array = [hex(x) for x in state]
    return hex_array

# Алгоритм хэширования
def stribog_hashing(m, N, Sigma, IV, M):
...
# Преобразование десятиричного числа в массив размером 64 из 8 битных чисел
def tov(n):
    strn = str(hex(n))[2:]
    strn = strn[::-1]
    ...
    vn = bytestringtolist(strn[::-1])
    vn = [int(value, 16) for value in vn]
    return vn
...
def stribog512(bmst):
    ...
    # Определение длинны входного слова(в битах)
    M = len(bmst)
    ...
    # Случай если длина блока больше чем 512 бит
    while len(mst) > 128:
        tmp = mst[-128:]
        N = addmod512(N, v512)
        Sigma = addmod512(Sigma, liststringtoint(bytestringtolist(tmp)))
        mst = mst[:len(mst) - 128]

    # Хэширование
    if len(mst) == 128:
        arrya = bytestringtolist(mst)
        m = [int(value, 16) for value in arrya]

        return stroutput(stribog_hashing(m, N, Sigma, IV, M))
    elif len(mst) < 128:
        binmst = hextobin(mst)
        binmst = "0" * (511 - len(binmst)) + "1" + binmst

        mst = bintohex(binmst)
        m = liststringtoint(bytestringtolist(mst))
...
def stribog256(bmst):
    ...
    if M == 512:
        ...
        ans = stroutput(stribog_hashing(m, N, Sigma, IV, M))[:64]
...
if __name__ == '__main__':
    ...
    type = int(input("What type of Streebog do you prefer, 256 or 512: "))

    #Выбор режима кодирования
    if type == 256:
        with open('output.txt', 'w') as file:
            # Запись строки в файл
            file.write(stribog256(binary_text))
        print(stribog256(binary_text))
    elif type == 512:
        with open('output.txt', 'w') as file:
            # Запись строки в файл
            file.write(stribog512(binary_text))
        print(stribog512(binary_text))
    else:
        raise ValueError("Value shold be equal 256 or 512")